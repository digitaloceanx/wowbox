...
function GridStatusRaidDebuff:OnInitialize()
	...
	self.db.profile.detect = false
	self.db.profile.detected_debuff = {}
end
...
-- Interface
local callStack = {};
function GridStatusRaidDebuff:Import(func)
	if (self.options) then
		func(self);
	else
		tinsert(callStack, func);
	end
end

function GridStatusRaidDebuff:DoImport()
	for i, func in ipairs(callStack) do
		func(self);
	end	
	callStack = {};
end
