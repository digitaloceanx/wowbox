...
--更新选项菜单
	if (RaidAlerter_MainFrame:IsVisible()) then
		...
		if RaidAlerter_SET.OTM_OnOff then RaidAlerterCheckLabel12_1:SetChecked(1);RaidAlerterCheckLabel13_1:Enable(); else RaidAlerterCheckLabel12_1:SetChecked(false);RaidAlerterCheckLabel13_1:Disable(); end
...
--事件注册注销并更新事件表RaidAlerter_EVENT_LIST
--[[
function RaidAlerter_EVENT_LIST_Update(evname,act)
	if act == "add" then
		RaidAlerter_Attack_Frame:RegisterEvent(evname);
		table.insert(RaidAlerter_EVENT_LIST,evname);
		return;
	end
	if act =="remove" then
		RaidAlerter_Attack_Frame:UnregisterEvent(evname);
		for k, v in pairs(RaidAlerter_EVENT_LIST) do
			if v == evname then
				table.remove(RaidAlerter_EVENT_LIST,k);
				return;
			end
		end
	end
...
dwStaticPopupDialogs["RaidAlerter_RELOADUI"] = {
...
function RaidAlerter_SET_FUNC_GTT_cpustart()
	if (GetCVar("scriptProfile") == "1") then
		...
		dwStaticPopup_Show("RaidAlerter_RELOADUI");
	end
end
...
function RaidAlerter_SET_FUNC_MsgTest()
	...
	local AlertMsg = {
		...
		[2]	= {
			["Raid"]=string.format(RAL_TEXT_ALERT_30,PN,RaidAlerter_RAIDICON[8]..RAL_TEXT_UINTNAME_3,RAL.SpellIdToText(48161,1)),
			["Scr"]=string.format(RAL_TEXT_ALERT_30,RAL.ToPink(RAL_TEXT_YOU),RaidAlerter_RAIDICON[8]..RAL.ToRed(RAL_TEXT_UINTNAME_3),RAL.ToCyan(RAL.SpellIdToText(48161,0,1))),
			},
		[3]	= {
...
function RaidAlerter_SET_FUNC_ResetToDefaut(arg)
	...
	RaidAlerter_SET_FUNC_BossHealthPecent(60);
...
function RaidAlerter_CheckRaidBuff()
	...
	--if GetBattlefieldInstanceRunTime()>0 or UnitInBattleground("player") then
...
--[[
		if (CureManaPercent > 0) and (ManaPercent > 0) and ((ManaPercent < 30) or (CureManaPercent < 20) or (DeadRaidmembers > RaidNum/2)) then
			RaidAlerter_Attack_Frame_Text:SetTextColor(1.0,0.1,0.1);
		else
			RaidAlerter_Attack_Frame_Text:SetTextColor(1.0,1.0,1.0);
		end
]]
...
--[[
function RaidAlerter_BU(s) local P,B,i="player",true,1 while UnitBuff(P,i) do if string.find(UnitBuff(P,i),s) then B=false end i=i+1 end return B end
function RaidAlerter_TargetBU(s) local P,B,i="target",true,1 while UnitBuff(P,i) do if string.find(UnitBuff(P,i),s) then B=false end i=i+1 end return B end
]]
function RaidAlerter_TargetDeBU(s,t) local B,i=true,1 while UnitDebuff(t,i) do if string.find(UnitDebuff(t,i),s) then B=false end i=i+1 end return B end

--统计有效成员数量（限定人数模式）
--[[
function RaidAlerter_GetNumGroupMembers(check)
	local n=0;
	for i=1, GetNumGroupMembers() do
		_, _, subgroup = GetRaidRosterInfo(i);
		if subgroup <= RaidAlerter_SET.RAIDMODE_MAX_GROUP then
...
--判定自己是否和团队在同一场景
--[[
function RaidAlerter_TestMe_InRaidZone()
	local MeZone = GetRealZoneText();
	local OnlineCount = 0;
	local MeZoneCount = 0;
	for i=1, GetNumGroupMembers() do
		...
		if subgroup <= RaidAlerter_SET.RAIDMODE_MAX_GROUP then
			if online then
				...
				if (MeZone==zone) then
					MeZoneCount = MeZoneCount + 1;
				end
			end
		end
	end
	if (MeZoneCount > (OnlineCount/2)) then
		return true;
	else
		return false;
	end
...
function RaidAlerter_IsBossUnit(unit)
	if UnitExists(unit) then
		...
		elseif GetNumGroupMembers() > 0 then
			...
			TeamLevel = TeamLevel/(GetNumGroupMembers()+1);
			PartyHPMAXAVG = PartyHPMAXAVG/(GetNumGroupMembers()+1);
			BossHPMin = PartyHPMAXAVG*15;			--PARTY
		end
		if EQDif >= 2 then
			BossHPMin = BossHPMin*1.35;
		end
--		if TeamLevel > 80 then
--			BossHPMin = BossHPMin*(1+((TeamLevel-80)/5));		--Lv80-85动态调整
--		end
...
--			if TeamLevel > 80 then
--				DangerUnitHPMin = DangerUnitHPMin*(1+((TeamLevel-80)/5));		--Lv80-85动态调整
--			end
...
function RaidAlerter_TestEnemyIsMe_Party()
	...
	if (Enemy_control_Count > GetNumGroupMembers()/2 ) then
		return true;
	else
		return false;
	end
...
--		local unit = RaidAlerter_NameToUnit(UnitName("raid"..id));
		local unit = "raid"..id;
		if (unit) then
			if (UnitIsFriend("player", "raid"..id) and UnitIsEnemy("player", "raid"..id)) then
...
function RaidAlerter_Check_P_BeControled_Party()
	...
	local BC_count = 0;
	local DEP_count = 0;
	for id=0, GetNumGroupMembers() do
		if (BC_count >= 3) or (DEP_count >= 3) then return; end			--一次最多只报3人，防刷屏
...
--查找锁定标记者ID
--[[
function RaidAlerter_Find_Hunter_SniperShot()
	if UnitExists("playertarget") then
		if(not UnitIsFriend("player","playertarget")) then
--				if (not RaidAlerter_TargetDeBU("Ability_Hunter_SniperShot","playertarget")) then
				if (not RaidAlerter_TargetDeBU(RAL_TEXT_BUFF_2,"playertarget")) then
					return "player";
				end
		end
	end
	if (UnitInRaid("player")) then
		for id=1, GetNumGroupMembers() do
			...
			if (UnitIsConnected(unit)) then
				if (not UnitIsDeadOrGhost(unit)) then
					if UnitExists("raid"..id.."target") then
							if(not UnitIsFriend("player","raid"..id.."target")) then
--								if (not RaidAlerter_TargetDeBU("Ability_Hunter_SniperShot",unit.."target")) then
								if (not RaidAlerter_TargetDeBU(RAL_TEXT_BUFF_2,unit.."target")) then
									return unit;
								end
							end
					end
...
--PET名称转换为ID
--[[
function RaidAlerter_PetNameToUnit(Name)
	...
	elseif (Name == UnitName("partypet4")) then
		...
		if (numRaidMembers > 0) then
			...
			for i=1, numRaidMembers do
				RaidPetName = UnitName("raidpet"..i);
				if ( Name == RaidPetName) then
					return "raidpet"..i;
				end
			end
		end
...
--				table.insert(RAL_BlzTanks, UnitName(unit));
...
--[[
function RaidAlerter_EndTimer(id)
	for k, v in pairs(RaidAlerterTimerDatas) do
		if(id == v["Name"]) then
			table.remove(RaidAlerterTimerDatas,k);
			return;
		end
	end
end
...
function RAL.SendMSG(msg, chan, ForceSend, CheckTime, MsgID)
	...
	if (chan=="RAID" or chan=="RAID_WARNING" or (chan=="INSTANCE_CHAT" and inautoparty == false)) then
		-- if select(2, IsInInstance()) == "pvp" and (not RaidAlerter_SET.TAQ_Twin_AOEAdd) then -- 战场禁用
		if select(2, IsInInstance()) == "pvp" and (not RaidAlerter_SET.TAQ_Twin_AOEAdd) then -- 战场禁用
		else
...
--[[
function RAL.ChatIconToSCRText(icontext, size)
	if (not icontext) or (icontext==nil) then return "";end
	for index,value in pairs(RaidAlerter_RAIDICON) do
		if icontext == value then
			return "|T".."Interface\\TargetingFrame\\UI-RaidTargetingIcon_"..index..":"..size.."|t"
		end
	end
	return icontext;
end
