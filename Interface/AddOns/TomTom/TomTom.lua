...
--Duowan interface
function Duowan_TomTom_Toggle_Closest(toggle)
	if toggle == 1 then toggle = true; else toggle = false; end
	TomTom.profile.poi.setClosest = toggle
	TomTom:EnableDisablePOIIntegration()
end

function Duowan_TomTom_Toggle_Block(toggle)
	if toggle == 1 then toggle = true; else toggle = false; end
	TomTom.profile.block.enable = toggle
	TomTom:ShowHideCoordBlock()    
end

function Duowan_TomTom_Toggle(toggle)
	TomTom:Initialize()
end
