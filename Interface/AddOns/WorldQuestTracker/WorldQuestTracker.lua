...
elseif (GetLocale() == "zhTW") then
	symbol_1K, symbol_10K, symbol_1B = "ä»Ÿ", "è¬", "å„„"
end
...
function WorldQuestTracker.SavedQuestList_IsNew (questID)
	...
	if (ActiveQuests [questID]) then --a quest esta armazenada
		if (map_seasons [questID] == WorldQuestTracker.MapSeason) then
			--a quest j?esta na lista porém foi adicionada nesta season do mapa
			return true
		else
			--apenas retornar que não ?nova
			return false
		end
...
hooksecurefunc ("ToggleWorldMap", function (self)
	...
	if (WorldMapFrame:IsShown()) then
		...
		if (not WorldMapFrame.firstRun and not InCombatLockdown()) then
			...
			WorldQuestButton:SetSize (80, 25)
			...
			WorldQuestButton:SetNormalFontObject("GameFontNormalSmall")
			WorldQuestButton:SetHighlightFontObject("GameFontHighlightSmall")
			WorldQuestButton:SetText(L["World Quest Tracker"])
		...
		--	EUI[1]:GetModule("Skins"):HandleButton(WorldQuestButton, true)
		...
		--	WorldQuestButton.Highlight:SetSize (WorldQuestButton.labText:GetWidth()*1.5, 32*1.5)
		--	WorldQuestButton.Highlight:SetPoint ("center")
			...
			local options_on_click = function (_, _, option, value, value2, mouseButton)
				...
				if (option == "tracker_is_locked") then
					--> s?aparece esta opção quando o tracker esta móvel
					if (WorldQuestTracker.db.profile.tracker_is_movable) then
						if (value) then
...
function WorldQuestTracker.RefreshTrackerWidgets()
	...
	--se não h?nenhuma quest sendo mostrada, hidar o cabeçalho
				...
				if (HaveQuestData (questID)) then
					...
					if (isWorldQuest) then
						...
						if (timeLeft and timeLeft > 0) then
							...
							if (timePriority) then --timePriority j?multiplicado por 60
