...
if (dwIsConfigurableAddOn("TDDps")) then
	dwRegisterCheckButton(
		...
		function (arg)			
			if (arg == 1) then
				dwLoadAddOn("TDDps");		
				TDDps_Options.enabled = true;
				TDDps_Frame:SetScript('OnUpdate', TDDps_OnUpdate);
			else
				if (IsAddOnLoaded("TDDps")) then
					TDDps_Options.enabled = false
	...
	dwRegisterCheckButton(
		...
		function (arg)			
			if (arg == 1) then
				...
				SetCVar("countdownForCooldowns", "0") --默认禁用BLZ自带的冷却数字
			else
