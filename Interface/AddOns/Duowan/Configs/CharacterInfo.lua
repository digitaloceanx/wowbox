	...
	dwRegisterCheckButton(
		...
		function (arg)
			if arg==1 then
				...
				ItemLevel_Toggle(true);
			else
				if (dwIsAddOnLoaded("TinyTip")) then	
					ItemLevel_Toggle(false);
				end				
			end
		end
	);	
...
if (dwIsConfigurableAddOn("GearStatsSummary")) then
	dwRegisterCheckButton(
		"ChracterInfo",
		GSS_TITLE,
		nil,
		...
		function (arg)
			if arg==1 then
			--	dwLoadAddOn("GearStatsSummary");	
				if (dwIsAddOnLoaded("GearStatsSummary")) then
					GSS_Toggle(true);
				end
			else
				if (dwIsAddOnLoaded("GearStatsSummary")) then	
