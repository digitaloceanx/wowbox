...
local nextInspectable = 0;	-- 可观察的时间
    ...
    for i=1, 17 ,1 do
        if GetInventoryItemTexture(unit, i) then
            ...
            if not link then
                ...
                if found then
                    if gem1~="0" and not GetItemGem(link, 1) then done = false end
                    if gem2~="0" and not GetItemGem(link, 2) then done = false end
                    if gem3~="0" and not GetItemGem(link, 3) then done = false end
                    if gem4~="0" and not GetItemGem(link, 4) then done = false end
                end
            end
        end
    end
