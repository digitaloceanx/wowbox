...
function AceComm:CHAT_MSG_SYSTEM(text)
	...
	_G.dwStaticPopupDialogs["ACECOMM_TOO_MANY_CHANNELS"] = {
		text = text,
		button1 = _G.CLOSE,
		timeout = 0,
		whileDead = 1,
	...
	_G.dwStaticPopup_Show("ACECOMM_TOO_MANY_CHANNELS")
end
...
local function activate(self, oldLib, oldDeactivate)
	...
	if not oldLib or not oldLib.hooks or not oldLib.hooks.ChatFrame_MessageEventHandler then
		...
		function _G.ChatFrame_MessageEventHandler(s, event, ...)
			if self.hooks.ChatFrame_MessageEventHandler then
				return self.hooks.ChatFrame_MessageEventHandler(self, old_ChatFrame_MessageEventHandler, s, event, ...)
			else
				return old_ChatFrame_MessageEventHandler(s, event, ...)
			end
		end
	end
