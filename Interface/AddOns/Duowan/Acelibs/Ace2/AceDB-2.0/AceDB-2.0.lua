...
function AceDB:DeleteProfile(profile, noconfirm)
	...
	if not (noconfirm or IsShiftKeyDown()) then
		if not dwStaticPopupDialogs["ACEDB20_CONFIRM_DELETE_DIALOG"] then
			dwStaticPopupDialogs["ACEDB20_CONFIRM_DELETE_DIALOG"] = {}
		end
		local t = dwStaticPopupDialogs["ACEDB20_CONFIRM_DELETE_DIALOG"]
		...
		dwStaticPopup_Show("ACEDB20_CONFIRM_DELETE_DIALOG")
		return;
	end
