...
local MINOR_VERSION = 90000 + tonumber(("$Revision: 1800 $"):match("(%d+)"))
...
local function confirmPopup(message, func, ...)
	if not dwStaticPopupDialogs["WATERFALL01_CONFIRM_DIALOG"] then
		dwStaticPopupDialogs["WATERFALL01_CONFIRM_DIALOG"] = {}
	end
	local t = dwStaticPopupDialogs["WATERFALL01_CONFIRM_DIALOG"]
	for k in pairs(t) do
		t[k] = nil
	end
	t.text = message
	...
	dwStaticPopup_Show("WATERFALL01_CONFIRM_DIALOG")
end
