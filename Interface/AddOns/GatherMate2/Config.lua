...
function Config:OnInitialize()
	...
	self.optionButton:SetHeight(20);
	self.optionButton:SetPoint("TOPRIGHT", WorldMapFrame, "TOPRIGHT", -140, -1);
	self.optionButton:SetFrameLevel(self.optionButton:GetFrameLevel() + 4);
	self.optionButton:SetText(L["GatherMate2"]);
	self.optionButton:SetScript("OnClick", function()
		LibStub("AceConfigDialog-3.0"):Open(L["GatherMate2"]);
	end);	
	if DataBroker then
